<?php
$nav = "
<ul class='nav nav-tabs p-3'>
	<li class='nav-item'>
		<a class='nav-link active' href='index.php?page=gallery'>Gallery</a>
	</li>
	<li class='nav-item'>
		<a class='nav-link' href='index.php?page=uploadForm'>Upload new image</a>
	</li>
</ul>
";
// $nav = "
// <ol class='breadcrumb'>
//     <li class='breadcrumb-item'><a href='index.php?page=gallery'>Gallery</a></li>
//     <li class='breadcrumb-item active' aria-current='page'><a href='index.php?page=uploadForm'>Upload new image</a></li>
//   </ol>
// ";

// $nav = "
// <nav class='py-3'>
// <a href='index.php?page=gallery'>Gallery</a>
// <a href='index.php?page=uploadForm'>Upload new image</a>
// </nav>
// ";
